...
- **Search results are checked for a pulse.** The directory's own health flag can be months stale — an entire national broadcaster moved streaming hosts in January, and in July every dead mount still read "checked fine", sailing straight past the broken-station filter. Every result shown now gets one real probe, cut off at the response headers: a definitive HTTP error tags the row "not answering" and dims it, so the eye skips what the ear would have had to discover. Slow or odd servers are left unjudged — a slow station is not a dead one — and a preview that fails in the ear tags its row the same way.
