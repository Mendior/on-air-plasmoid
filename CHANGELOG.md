...
- **The remembered-speed list is bounded.** The per-show playback-speed memory
  ...
  limit.
- **Search speaks geography.** Type `70s in UK` or `jazz from France` and the
  search runs inside that country — names, genres and the word-order pass all
  scoped, with paging to match. The country pins as a flag chip: follow-up
  queries stay inside it until its ✕ releases them, and a bare country search
  or Trending hands the country role back honestly. Requested by a listener on
  KDE Discuss.
- **Two new languages: Finnish and Hindi.** The widget now ships in thirteen
  languages — suomi joins as the home-country language, and हिन्दी for the
  sizeable Indian listener community.
- **The world grew flags and a dice.** Every search result now carries its
  country's flag (emoji, no image assets). A `Popular in <your country>`
  shortcut reads the system locale — nothing asked, nothing sent — and the
  🎲 *Surprise me* button tunes into a random country's top station: press it
  again to travel on, star what you want to keep.

## 2026.22

The podcatcher release. The popup grew a map — five labeled tabs — and the Podcasts tab behind it grew from a search box into a podcatcher that stands next to the dedicated apps: three directories, instant streaming, chapters, an Up-next queue, self-refreshing subscriptions that download and clean up after themselves, and a wake-up alarm that plays your show. I picked it apart hard before tagging it, and everything I managed to break on purpose is now held down by a test.
...
The hardening release. I went back over the whole codebase hunting for the ways this widget could misbehave or betray you, fixed what I found, and tripled the test suite while I was in there. Short version below; the commit history tells each story in full.
