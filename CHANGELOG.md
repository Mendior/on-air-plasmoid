...
- **A second, deeper sweep — every file, top to bottom.** The Settings "Search stations" tab had a load-breaking error that left it blank (no automated check ever opened a settings page — now one compiles all four on every build). Wake-up alarms and scheduled recordings both stay honest across a daylight-saving change, including one that happens while the computer is off, and a network blip during a ringing alarm no longer drops it back to bedtime volume. Search results are checked for a real pulse without ever double-knocking a host or letting the verdict cache grow forever; an inflected name like "Elmari" finds "Raadio Elmar"; and a dead result quietly hands off to its living twin. Under the hood: the cast bridge can't be hung by a dripping renderer or pointed at local files, a killed recorder is reaped instead of left a zombie, the microphone check refuses a room that is too loud to trust, station rows answer the keyboard and screen readers, catalog artwork can't flood memory, and the translation catalogs no longer leak a local path or ship an unchecked placeholder. Nothing here changes how the app looks; it changes how little can go wrong.
...
- **Search results are checked for a pulse.** The directory's own health flag can be months stale — an entire national broadcaster moved streaming hosts in January, and in July every dead mount still read "checked fine", sailing straight past the broken-station filter. Every result shown now gets one real probe, cut off at the response headers, and only the codes stream hosts actually use for a mount that is gone — not found, forbidden, gone — tag the row "not answering" and dim it. Everything else stays unjudged: server hiccups, throttling codes (a living national station was measured answering a non-standard 460 to every fresh connection the moment a CDN rate limiter woke up), slow servers, legacy ICY relics. Verdicts are remembered for a quarter hour, so retyping a query never re-knocks on the same thirty hosts — the widget can never work a host up to that rate limiter itself. A preview that fails in the ear tags its row the same way.
- **A dead result gets one more real chance.** The same directory very often lists the same station twice — one entry pointing at the abandoned host, one at wherever it streams today. When a preview's address is dead and even the directory's current record for it has rotted, a name search now finds the living twin and plays it — ranked exactly like the station-list heal: exact name first, the station's own domain above everything. Only when no twin answers does the honest error speak.
- **Search by name behaves like searching by a name.** Words match in any order ("nova radio" finds Radio Nova — the directory itself only does substrings), accents no longer hide stations ("jarvi" finds Järviradio), and the station whose name IS the query rises above the merely famous instead of drowning under them. An inflected query no longer comes back empty-handed either: "Elmari" finds Raadio Elmar — a search with no results retries with up to two trailing letters shaved off before conceding. The station list's own filter went accent-blind the same way.
...
- **The settings tabs speak your language — for the first time.** The four category names (Stations, Appearance, Recording, Search) had never entered the translation catalogs; they were English in all eleven languages since the first release. Weekday names, unit suffixes and six glued-together sentences joined the catalogs too, and every language ships complete: 352 of 352 strings.
...
- **The grey silhouette stopped posing as album art.** Deezer serves a placeholder image for artists it has no picture of, and its second spelling — a valid-looking URL wearing the hash of an empty file — slipped past the existing filter, got cached, and sat where a cover should be for the rest of the session. Both spellings are rejected now, so an unmatched track falls through honestly to the station's own logo.
